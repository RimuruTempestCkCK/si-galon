...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($error === "") {
        ...
        // Prepared statement insert (sesuaikan kolom orders di DB)
            $stmt = $conn->prepare("INSERT INTO orders (user_id, order_code, quantity, recipient_name, phone, latitude, longitude, notes, total) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)");
        ...
        if (!$stmt) {
            ...
            if ($stmt->execute()) {
                header("Location: verifikasi-pembayaran copy.php?order_code=" . urlencode($order_code));
                exit;
            } else {
...
function setLocationType(type) {
    locationType = type;
    document.getElementById('manualLocation').style.display = type === 'manual' ? 'block' : 'none';
    if(type === 'gps') {
        initMap();
    }
}
